...
| 10.10.30.28/30   | 10.10.30.0/24  | 20AA:BBCC:30:28::/64     | 20AA:BBCC:20::/48   | Питер AS 2042 | SW9e0/0  SW10e0/0 |
...
5.  Данная команда отключает процес обмена EIGRP пакетами на всех интерфейсах.
 ...
 Для примера использован маршрутизатор R32.
...
В таблице присутствуют сети 10.10.30.0 - 4 - 8 - 12 - 16  с маской сети 30

Для выполнения суммирования необходимо на маршрутизаторах R16 и R17 на интерфесах смотрящих на R18 выполнить следующие команды: 
     ...
     R16-17(config-router-af-interface)#summary-address 10.10.30.0 255.255.255.240
     
 Для суммирования маршрутов использовалась 28 маска (255.255.255.240). Если использовать 27 маску, то под ее значение попадут connected сети роутера R18.

После выполнения данных команд роутер R18 имеет измененную таблицу маршрутизации:
