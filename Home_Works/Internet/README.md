...
и интерфейс e0/1 смотрящий на R15 как ip nat outside. Это необходимо так как трафик в Интернет идет через R15 согласно предыдущего задания и все роутеры в Интернет поcылают так же трафик на R15.  
 ...
 ##  6. Настроите для IPv4 DHCP сервер в офисе Москва на маршрутизаторах R12 и R13. VPC1 и VPC7 должны получать сетевые настройки по DHCP
  ...
  для сети LAN_20
   ...
   На скриншотах ниже показаны IP адреса полученные по DHCP при включенном роутере R12 и отключеном R13. И наоборот
 ...
 ## 7. Настроите NTP сервер на R12 и R13. Все устройства в офисе Москва должны синхронизировать время с R12 и R13
   ...
   Настроим R23 Триада как NTP server в Интернет со Stratum 1. IP адрес для обращения будет 150.150.150.1 - это PI адрес Триады. R23 взят просто в качестве примера. 
   
        ntp master 1 
   
 Для выполнения задания необходимо:
 
 1. На роутерах офиса Москвы задаем временную зону MSK +3 часа
   
          сlock timezone MSK 3
      
 2. На роутерах R12 и R13 настроим обращение к NTP серверу в Интернет. В моем случае к R23 150.150.150.1
      ...
      100.100.20.13/32 - это PI адрес AS1001 созданный на loopback1 R13 который доступен из Интернет 
        ...
        ntp source loopback 1 - указывается интерфейс из под которого будет обращение в Интернет для получения ntp данных 
        ntp server 150.150.150.1 prefer - указывает NTP сервер как предпочтительный
        ntp master 2 - команда делает R12 и R13 в качесвте ntp серверов к которым будет обращение из Офиса Москвы
 ...
 3. На роутерах Москвы выполняем следующие команды:
 
        ntp source Loopback0 - указывается интерфейс из под которого будет обращение к локальному NTP серверу. Адреса на loopback0 доступны по OSPF внутри AS1001
        ntp server 12.12.12.12 prefer - IP Loopback R12 доступный по OSPF. Данный сервер будет предпочтительный в AS1001
        ntp server 13.13.13.13 - IP Loopback R13 доступный по OSPF. Запасной сервер в случае отказа R12
