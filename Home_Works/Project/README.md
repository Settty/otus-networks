...
Далее на всех Spoke маршрутизаторах будут аналогичные команды для установления соседства BGP 

    
    router bgp 60200
    neighbor 172.16.100.1 remote-as 60100 - установка BGP с Hub_1
    neighbor 172.16.100.1 allowas-in - команда необходимая для игнорирования механизма loop prevention. По скольку все Spoke находятся в одной AS, update не будут распространяться между Spoke по умолчанию. Данная команда снимает эти ограничения
    neighbor 172.16.200.1 remote-as 60100 - установка BGP с Hub_2
    neighbor 172.16.200.1 allowas-in - аналогично описанию выше
    network 192.168.30.0 - анонсируем сети офиса R12. На дргих Spoke будут анонсированы другие сети, но принцип тот же.
    network 192.168.40.0 - анонсируем сети офиса R12
    
 
![](R12_bgp.png)

Как видно из скриншота установилось BGP соседство с Hub_1 (172.16.100.1) и Hub_2 (172.16.200.1). Так же в столбце State/PsxRcd видно что Spoke получает по одному префиксу от каждого Hub. Хотя в AS_60100 присутсвуют несколько офисных сетей. Так происходит потому, что на Hub маршрутихаторах в настройках BGP была выполнена команда суммаризации маршрутов **aggregate-address 192.168.0.0 255.255.0.0 summary-only**

Проверим IP связанность офисных сетей между Spoke_1 R12 и R1 Hub_1 - R2 Hub_2

![](ping_1.png)

Из скриншота видно, что пинг от офиса Spoke_1 к головному офису проходит по туннелю через Hub_1. Но трафик на данный момент не зашифрован.

####  Для шифрования трафика необходимо использовать группу протоколов IPSec.

На Hub_1 и Hub_2 выполним следующие настройки

1 Фаза IPSec в которой создается мини туннель для обмена ключами и некоторыми необходимыми данными
  
    crypto isakmp policy 1 - создаем политику 1 фазы
    encr aes - устанавливаем шифрование данных с помошью симметричного протокола AES 
    authentication pre-share - аутентификация будет про ключу
    group 14 - обмен ключами про протоколу DH 2048 bit
    lifetime 3600 - изменение ключа каждый час
    crypto isakmp key 12345 address 0.0.0.0 - сам ключ 12345. Адрес 0.0.0.0 говорит о том, что Hub_1 будет устанавливать политики 1 фазы с любым Spoke       
    crypto isakmp keepalive 15 periodic - IPSec Dead Peer Detection - механизм обнаружения неактивных IPSec SA security association 
...
настройка профиля IPSec
 ...
 Далее привязываем созданный профиль на туннельный интерфейс Hub_1 и Hub_2
 
    int Tun 1
    tunnel protection ipsec profile IPsec
    
 Настроим IPSec на Spoke_1 R12. Настройки аналоичные как на Hub_1 и Hub_2 за исключением некоторых нюансов
 
    crypto isakmp policy 1
    encr aes 
    authentication pre-share
    group 14 
    crypto isakmp key 12345 address 100.100.100.1 - для установки мини-туннеля 1 фазы с Hub_1
    crypto isakmp key 12345 address 100.100.100.2 - для установки мини-туннеля 1 фазы с Hub_2  
...
Так же "вешаем" профиль IPSec на оба туннельных интерфейса Spoke_1 

    Int Tun 1
    tunnel protection ipsec profile IPSec shared - ключевая команда shared позволяющая использовать общие политики IPSec SA. Без нее созданный профиль IPSec можно применить только на один туннельный интерфейс
    
    Int Tun 2
    tunnel protection ipsec profile IPSec shared
    
 После этого пропингуем с офиса Spoke_1 (R12) головной офис
 
 ![](ping_2Sec.png) 
 
 Как видно из скриншота, пакеты идут зашифрованные протоколом ESP.

 Так же проверим установку Фазы 1 и Фазы 2
 
 ![](phase1.png) 
 
 ![](phase2.png)
 
 Видно, что во второй фазе пакеты шифруются.
...
Далее настраиваем BGP 
 ...
 Записи Outide local и Outside global это IP Hub_1 и Hub_2
 
 Так же если обратиться к таблицам NHRP Hub_1 и Hub_2, то можно увидеть соответствующие записи **claimed** Это серый адрес маршрутизатора Spoke_2 находящегося за NAT R8, а NBMA адрес это сам белый адрес маршрутизатора R8
 
 ![](R1_nhrp.png)
 
 ![](R2_nhrp.png)
 
Соединение DMVPN устанавливается без проблем. 

Далее настроим третий офис Spoke_3 c аналогичными настройками, который будет DHCP сервером для офисных сетей. Так делать не желательно, но хочу показать, что DMVPN работает во всех филиалах.

Схема следующая: R22 получает белый маршрутизированный Ip 7.22.7.22. Из под этого адреса будут строиться туннели.

Настраиваем оба туннеля для соединения со Spoke_1 и Spoke_2. 

     tunnel 1 - 172.16.100.4
     tunnel 2 - 172.16.200.4
     
Настраиваем BGP и фазы IPsec

Далее настраиваем DHCP сервер.

    Создаем исключения для шлюзов офисных сетей
...
Далее на каждом маршрутизаторе где созданы офисные сети прописать helper-address DHCP сервера.
 ...
 На обоих Hub появились записи в NHRP таблице 
 
 ![](R1_3.png)
 
 ![](R2_3.png)
      
Проверим на нескольких офисных Пк, что они получают адрес по DHCP
...
Далее для проверки отказоустойчивости отключим Hub_1. 

![](PC6.png)

Из скриншота видно, что трасировка идет из подсети офиса Spoke_2 через Hub_1 172.16.100.1. Далее происходит отключение и через некоторое время трафик начинает ходить через Hub_2 172.16.200.1

## Цели и задачи обозначенные в данном проекте достигнуты, а именно:

### Организовали связь DmVPN Phase 3
### Настроили протокл динамической маршрутизации BGP с возможностью суммариазции на Hub
### Защитили трафик с помошью группы протоколов IPSec
### Создали отказоустойчивость с помошью нескольких Hub c топологией Dual Hub Dual Cloud
